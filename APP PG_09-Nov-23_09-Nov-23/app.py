...
db_dir = 'C:/Users/PRANAV SINGH/Desktop/APP PG_09-Nov-23_09-Nov-23/APP PG_09-Nov-23_09-Nov-23'
...
def insert_data(name, reg_number, password):
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO passwords (name, reg_number, password) VALUES (?, ?, ?)', (name, reg_number, password))
    conn.commit()
    conn.close()
...
def generate_password():
    ...
    for _ in range(num_combinations):
        ...
        insert_data(name, reg_number, password)
        generated_results.append({'name': name, 'reg_number': reg_number, 'password': password})

    return render_template('results.html', generated_results=generated_results)
